...
void loadListviewItemAt(LPCWSTR path, HWND m_hParent, HWND m_hListView, TroGiup *drive);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		hicon = LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_DESKTOP));
		ThisPCIcon = ImageList_AddIcon(himl, hicon);
		DeleteObject(hicon);

		hicon = LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_MYCOMPUTER));
		...
		for (int i = 0; i < g_Drive->getCount(); ++i)
		{
			...
			tvInsert.item.lParam = (LPARAM)g_Drive->getLetter(i);
		...
		for (int i = 0; i < g_Drive->getCount(); ++i)
		{
			...
			lv.lParam = (LPARAM)g_Drive->getLetter(i);
			...
			lv.iSubItem = 2;
			if (wcscmp(g_Drive->getType(i), CD_ROM) != 0)
...
void loadDirItemToLisview(HWND m_hParent, HWND m_hListView, LPCWSTR path)
{
	...
	do
	{
			...
			((fd.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN) != FILE_ATTRIBUTE_HIDDEN))
		{
			...
			ListView_SetItemText(m_hListView, itemIndex, 3, KichThuoc::convertByteToStringSize(fileSizeLow));
	...
	SendMessage(GetDlgItem(m_hParent, IDC_STATUSBAR), SB_SETTEXT, 0, (LPARAM)KichThuoc::convertByteToStringSize(fileSizeCount));
}
